...
while True:
    ...
    class_names = ["with helmet", "without helmet", "rider", "number plate", "traffic light"]
    ...
    for r in results:
        for box in r.boxes:
            ...
            if len(track_history[obj_id]) >= 2:
                ...
                if direction < -20:
                    cv2.putText(frame, "WRONG SIDE!", (x1, y2+40),
            ...
            if y2 > LINE_Y and red_light:
                ...
                if current_time - last_saved_time > 3:
                    ...
                    cv2.putText(frame, "RED LIGHT VIOLATION!", (x1, y2+20),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,0,255), 2)

                    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                    filename = f"{save_path}/violation_{timestamp}.jpg"
